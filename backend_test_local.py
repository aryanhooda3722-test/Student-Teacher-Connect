...
Using local backend for testing due to external URL timeout issues
"""
...
def test_user_authentication():
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/auth/login", 
                               json=invalid_login, headers=HEADERS, timeout=TIMEOUT)
...
def test_user_profile_management(teacher_token, student_token):
    ...
    if teacher_token:
        try:
            ...
            response = requests.put(f"{BASE_URL}/users/me", 
                                  json=update_data, headers=auth_headers, timeout=TIMEOUT)
            if response.status_code == 200:
                data = response.json()
...
def test_assignment_management(teacher_token, student_token):
    """Test assignment CRUD operations and role-based access"""
    results = TestResults()
    assignment_id = None
    ...
    if student_token:
        try:
            auth_headers = {**HEADERS, "Authorization": f"Bearer {student_token}"}
            response = requests.get(f"{BASE_URL}/assignments", headers=auth_headers, timeout=TIMEOUT)
            if response.status_code == 200:
                data = response.json()
    ...
    if teacher_token:
        try:
            auth_headers = {**HEADERS, "Authorization": f"Bearer {teacher_token}"}
            response = requests.get(f"{BASE_URL}/assignments/my", headers=auth_headers, timeout=TIMEOUT)
            if response.status_code == 200:
                data = response.json()
                if isinstance(data, list):
...
def test_assignment_completion(student_token, teacher_token, assignment_id):
    ...
    if student_token:
        try:
            auth_headers = {**HEADERS, "Authorization": f"Bearer {student_token}"}
            response = requests.get(f"{BASE_URL}/submissions/my", headers=auth_headers, timeout=TIMEOUT)
            if response.status_code == 200:
                data = response.json()
...
def main():
    ...
    auth_results, teacher_token, student_token, teacher_data, student_data = test_user_authentication()
    overall_results.passed += auth_results.passed
    overall_results.failed += auth_results.failed
    overall_results.errors.extend(auth_results.errors)
    
    # Test 2: User Profile Management
    print("\n👤 Testing User Profile Management...")
    profile_results = test_user_profile_management(teacher_token, student_token)
    ...
    if overall_results.failed == 0:
        ...
        print("✅ Assignment Management System - Working") 
        print("✅ Assignment Completion System - Working")
        print("✅ Database Models and MongoDB Integration - Working")
